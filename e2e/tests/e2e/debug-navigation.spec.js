...
test.describe('Debug Navigation', () => {
  test('should navigate to horarios page successfully', async ({ authenticatedPage }) => {
    ...
    const titleVisible = await authenticatedPage.locator('h1:has-text("Horarios")').isVisible();
    console.log('Title visible:', titleVisible);
    
    if (titleVisible) {
      const titleText = await authenticatedPage.locator('h1:has-text("Horarios")').textContent();
